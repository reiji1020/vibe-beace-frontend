...
import { POST } from '../../src/routes/api/update/setWishlistBead/+server';
import { setWishlistBead } from '$lib/controllers/beadController';
...
describe('API setWishlistBead', () => {
  let cookies: Cookies;
  let token: string;
  beforeEach(() => {
    cookies = createCookies();
    token = ensureCsrfCookie(cookies);
    (setWishlistBead as any).mockClear?.();
  });
  ...
  it('updates wishlist with 200 on success', async () => {
    const req = new Request('http://localhost', {
      ...
      body: JSON.stringify({ id: 9, wishlist: false })
    });
    const res = await POST({ request: req, cookies } as any);
    const out = await read(res);
    expect(out.status).toBe(200);
    expect(out.body.success).toBe(true);
    expect((setWishlistBead as any).mock.calls[0]).toEqual([9, false]);
  });
  ...
  it('returns 403 on invalid CSRF', async () => {
    const req = new Request('http://localhost', {
      ...
      body: JSON.stringify({ id: 9, wishlist: true })
    });
    const res = await POST({ request: req, cookies } as any);
    const out = await read(res);
  ...
  it('returns 500 when controller throws', async () => {
    (setWishlistBead as any).mockImplementationOnce(async () => { throw new Error('boom'); });
    const req = new Request('http://localhost', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': token },
      body: JSON.stringify({ id: 9, wishlist: true })
