...
vi.mock('../../src/lib/controllers/cutClothController', () => ({
  updateCutCloth: vi.fn(async (data) => ({ ...data }))
}));

import { POST } from '../../src/routes/api/update/updateCutCloth/+server';
import { updateCutCloth as updateCutClothMock } from '$lib/controllers/cutClothController';
...
describe('API updateCutCloth', () => {
  ...
  it('updates cut cloth with 200 when valid', async () => {
    const req = new Request('http://localhost', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': token },
      body: JSON.stringify({ id: 12, ...base })
    });
  ...
  it('returns 400 on zod error', async () => {
    const req = new Request('http://localhost', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': token },
      body: JSON.stringify({ id: 12, ...base, quantity: -1 })
    });
    const res = await POST({ request: req, cookies } as any);
    const out = await read(res);
    expect(out.status).toBe(400);
  ...
  it('returns 500 when controller throws', async () => {
    (updateCutClothMock as any).mockImplementationOnce(async () => { throw new Error('boom'); });
    const req = new Request('http://localhost', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': token },
      body: JSON.stringify({ id: 12, ...base })
