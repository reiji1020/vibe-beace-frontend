...
import { POST } from '../../src/routes/api/update/setWishlistXStitchCloth/+server';
import { setWishlistXStitchCloth } from '$lib/controllers/xStitchClothController';
...
describe('API setWishlistXStitchCloth', () => {
  let cookies: Cookies;
  let token: string;
  beforeEach(() => {
    cookies = createCookies();
    token = ensureCsrfCookie(cookies);
    (setWishlistXStitchCloth as any).mockClear?.();
  });
  ...
  it('updates wishlist with 200 on success', async () => {
    const req = new Request('http://localhost', {
      ...
      body: JSON.stringify({ id: 14, wishlist: false })
    });
    const res = await POST({ request: req, cookies } as any);
    const out = await read(res);
    expect(out.status).toBe(200);
    expect(out.body.success).toBe(true);
    expect((setWishlistXStitchCloth as any).mock.calls[0]).toEqual([14, false]);
  });
  ...
  it('returns 400 on invalid body', async () => {
    const req = new Request('http://localhost', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': token },
      body: JSON.stringify({ id: -2, wishlist: false })
    });
    const res = await POST({ request: req, cookies } as any);
    const out = await read(res);
  ...
  it('returns 500 when controller throws', async () => {
    (setWishlistXStitchCloth as any).mockImplementationOnce(async () => { throw new Error('boom'); });
    const req = new Request('http://localhost', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': token },
      body: JSON.stringify({ id: 14, wishlist: false })
