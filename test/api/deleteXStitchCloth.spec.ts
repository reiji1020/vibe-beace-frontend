...
import { DELETE } from '../../src/routes/api/delete/deleteXStitchCloth/+server';
import { deleteXStitchCloth as deleteXStitchClothMock } from '$lib/controllers/xStitchClothController';
...
describe('API deleteXStitchCloth', () => {
  let cookies: Cookies;
  let token: string;
  beforeEach(() => {
  ...
  it('deletes xstitch cloth with 200', async () => {
    ...
    expect(out.body.data.id).toBe(31);
  });
  ...
  it('returns 500 when controller throws', async () => {
    ...
    (deleteXStitchClothMock as any).mockImplementationOnce(async () => { throw new Error('boom'); });
    const req = new Request('http://localhost', {
      method: 'DELETE',
      headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': token },
      body: JSON.stringify({ id: 31 })
